...
def weight_scaling_dense(x, filters, gain, use_pixelnorm=False, activate=None, use_bias=True, use_batchnorm=True):
...
def weight_scaling_conv(x, filters, kernel_size, gain, use_pixelnorm=False, activate=None, strides=(1,1), use_bias=True, use_batchnorm=True):
    init = RandomNormal(mean=0., stddev=0.02)
    in_filters = backend.int_shape(x)[-1]
...
def weight_scaling_transposed_conv(x, filters, kernel_size, gain, use_pixelnorm=False, activate=None, strides=(1,1), use_bias=True, use_batchnorm=True):
    init = RandomNormal(mean=0., stddev=0.02)
    in_filters = backend.int_shape(x)[-1]
...
class PCGAN(Model):
    ...
    def init_discriminator(self):
        img_input = layers.Input(shape = (64,64,1), dtype=tf.float32)

        label_input = layers.Input(shape = (1,))
        label_embedding = layers.Embedding(self.num_classes, 64*64)(label_input)
        #label_embedding = layers.Dense(64*64)(label_embedding)

        label = layers.Reshape((64, 64, 1))(label_embedding)

        #label = layers.Reshape((1, 1, self.num_classes))(label_input)
        #label = layers.UpSampling2D(64)(label)

        concat_input = layers.Concatenate()([img_input, label])

        x = weight_scaling_conv(concat_input, filters=self.filters[3], kernel_size=(5,5), gain=np.sqrt(2), activate='LeakyReLU', strides=2)
        x = weight_scaling_conv(x, filters=self.filters[2], kernel_size=(5,5), gain=np.sqrt(2), activate='LeakyReLU', strides=2)
        x = weight_scaling_conv(x, filters=self.filters[1], kernel_size=(5,5), gain=np.sqrt(2), activate='LeakyReLU', strides=2)
        x = weight_scaling_conv(x, filters=self.filters[0], kernel_size=(5,5), gain=np.sqrt(2), activate='LeakyReLU', strides=2)
        #x = weight_scaling_conv(x, filters=1, kernel_size=(4,4), gain=np.sqrt(2), strides=1)
    ...
    def init_generator(self):
        ...
        label_embedding = layers.Embedding(self.num_classes, 4*4*64)(label)
        #label_embedding = layers.Dense(4*4*64)(label_embedding)
        label_embedding = layers.Reshape((4,4,64))(label_embedding)

        #concat_input = layers.Concatenate()([noise, label])

        x = weight_scaling_dense(noise, filters=4*4*self.filters[0], gain=np.sqrt(2)/4, activate='LeakyReLU', use_pixelnorm=True)
        x = layers.Reshape((4, 4, self.filters[0]))(x)

        x = layers.Concatenate()([x, label_embedding])

        x = weight_scaling_transposed_conv(x, filters=self.filters[1], kernel_size=(5,5), gain=np.sqrt(2), activate='LeakyReLU', use_pixelnorm=True, strides=2)
        x = weight_scaling_transposed_conv(x, filters=self.filters[2], kernel_size=(5,5), gain=np.sqrt(2), activate='LeakyReLU', use_pixelnorm=True, strides=2)
        x = weight_scaling_transposed_conv(x, filters=self.filters[3], kernel_size=(5,5), gain=np.sqrt(2), activate='LeakyReLU', use_pixelnorm=True, strides=2)
        x = weight_scaling_transposed_conv(x, filters=1, kernel_size=(5,5), gain=np.sqrt(2), activate='tanh', use_pixelnorm=True, strides=2)
